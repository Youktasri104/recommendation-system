...
def helper(dis):
    desr = des[des['Disease'] == dis]['Description']
    desr = " ".join([c for c in desr])

    preca = prec[prec['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    preca = [d for d in preca.values]

    med = medi[medi['Disease'] == dis]['Medication']
    med = [e for e in med.values]

    die = diet[diet['Disease'] == dis]['Diet']
    die = [f for f in die.values]

    work = workout[workout['disease'] == dis]['workout']
    work = [g for g in work.values]

    return desr, preca, med, die, work


sym_dic = {'itching': 0, 'skin_rash': 1, 'nodal_skin_eruptions': 2, 'continuous_sneezing': 3, 'shivering': 4,
...
def predict_value(symptom):
    a = np.zeros(len(sym_dic))
    for i in symptom:
        a[sym_dic[i]] = 1
    return diseases_list[svc.predict([a])[0]]



#sym=pd.DataFrame(sym_list)
st.title(
    'Disease prediction and Medicine recommendation system'
)


symptoms = st.text_input("Enter your symptoms", "__")
#st.write("The current movie title is", symptoms)


if st.button("Search"):
    #st.write("symptoms your having")
    user = [j.strip() for j in symptoms.split(',')]
    user = [b.strip("[] '") for b in user]
    predict = predict_value(user)
    desr, preca, med, die, work = helper(predict)
